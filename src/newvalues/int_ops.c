...
fus_value_t fus_int_add(fus_vm_t *vm,
    ...
    if((x > 0) && (y > FUS_PAYLOAD_MAX - x))return fus_err(vm, FUS_ERR_OVERFLOW);
    if((x < 0) && (y < FUS_PAYLOAD_MIN - x))return fus_err(vm, FUS_ERR_UNDERFLOW);
...
fus_value_t fus_int_mul(fus_vm_t *vm,
    ...
    /* overflow/underflow checks */
    /* Taken from https://stackoverflow.com/a/1514309 */
    if(y > FUS_PAYLOAD_MAX / x)return fus_err(vm, FUS_ERR_OVERFLOW);
    if(y < FUS_PAYLOAD_MIN / x)return fus_err(vm, FUS_ERR_UNDERFLOW);
    /* The following checks are apparently only necessary on two's compliment machines */
    if((x == -1) && (y == FUS_PAYLOAD_MIN))return fus_err(vm, FUS_ERR_OVERFLOW);
    if((y == -1) && (x == FUS_PAYLOAD_MIN))return fus_err(vm, FUS_ERR_OVERFLOW);
