...
void fus_obj_init(fus_obj_t *o, fus_vm_t *vm){
    fus_arr_init(&o->keys, vm);
    fus_arr_init(&o->values, vm);
}

void fus_obj_copy(fus_obj_t *o, fus_obj_t *o2){
    /* Acts like obj_init for o */
    fus_arr_copy(&o->keys, &o2->keys);
    fus_arr_copy(&o->values, &o2->values);
}

void fus_obj_cleanup(fus_obj_t *o){
    fus_arr_cleanup(&o->keys);
    fus_arr_cleanup(&o->values);
}
...
void fus_boxed_obj_mkunique(fus_boxed_t **p_ptr){
    ...
    if(p->refcount > 1){
        ...
        fus_obj_copy(&new_p->data.o, &p->data.o);
        *p_ptr = new_p;
    }
}
